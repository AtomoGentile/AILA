 ...
 * - Giro periodico delle circolari: [runRefresh], chiamato dal `BGAppRefreshTask` di AppDelegate.
 */
object IosBackgroundWork {
    ...
    /**
     * Un giro di [BackgroundCircularsSync] nel tempo che iOS concede a un `BGAppRefreshTask`
     * (circa 30 secondi): al massimo due circolari, e si chiude comunque prima dello scadere.
     * [onDone] viene chiamata una sola volta, sul thread principale, con l'esito.
     */
    fun runRefresh(onDone: (Boolean) -> Unit): BackgroundRun {
        val job = scope.launch {
            val ok = try {
                withTimeout(REFRESH_BUDGET_MILLIS) {
    ...
    private const val REFRESH_BUDGET_MILLIS = 25_000L
}
